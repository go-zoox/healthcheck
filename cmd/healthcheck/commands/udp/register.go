...
func Register(app *cli.MultipleProgram) {
	app.Register("udp", &cli.Command{
		Name:  "udp",
		Usage: "udp is a command that checks udp port health.",
		Flags: []cli.Flag{
			&cli.Int64Flag{
				Name:  "timeout",
		...
		Action: func(ctx *cli.Context) error {
			...
			if ok, err := healthcheck.UDP(ip, port, time.Duration(ctx.Int64("timeout"))*time.Millisecond); err != nil {
				return err
			} else if !ok {
